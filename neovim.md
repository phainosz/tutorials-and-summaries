...
`f` move o cursor para frente na palvra/símbolo buscado.
  - `f*` move o cursor para no * encontrado.
...
`Ctrl + o` Quando você se move para uma nova localização no código, como ao usar o comando gd (go to definition), Neovim adiciona essa posição à lista de saltos. Pressionar `Ctrl-o` permite que você volte para a posição onde estava antes do salto.
